...
fun createRandomBrick(game: Game): Position = when (game.hacking.level) {
    ...
    2 -> when {
        (game.apple != null && game.hacking.golden != null) ->
    ...
    3 -> when {
        game.apple != null && game.hacking.poison != null ->
            (ALL_POSITIONS - game.hacking.poison - game.snake.body - game.apple - game.wall).random()
        game.apple != null && game.hacking.poison == null ->
            (ALL_POSITIONS - game.snake.body - game.apple - game.wall).random()
        game.apple == null && game.hacking.poison != null ->
            (ALL_POSITIONS - game.snake.body - game.hacking.poison - game.wall).random()
        else -> (ALL_POSITIONS - game.snake.body - game.wall).random()
    }
...
fun initBlocksTwo(): List<Position> {
    ...
    } + line2.map { Position(it, 2) } + collum16.map { Position(16, it) } + collum3.map {
        Position(3, it)
